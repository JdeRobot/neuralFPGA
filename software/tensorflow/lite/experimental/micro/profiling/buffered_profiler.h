...
class BufferedProfiler : public tflite::Profiler {
 public:
  ...
  uint32_t BeginEvent(const char* tag, EventType event_type, uint32_t event_metadata) override {
    if (!enabled_) {
    ...
    event_buffer_[index].begin_timestamp = clock();
  ...
  void EndEvent(uint32_t event_handle) override {
    ...
    event_buffer_[event_handle].end_timestamp = clock();
  }
