#include "VTote_sim.h"
#include "VTote_sim_Tote_sim.h"
#include "VTote_sim_Bram.h"
#include "VTote_sim_Tote.h"
#include "VTote_sim_VexRiscv.h"
#include "testbench.h"
...
class VexRiscvTracer : public Agent {
 public:
  VTote_sim_VexRiscv *cpu;
  bool trace_instruction;
  bool trace_reg;
  std::ofstream instructionTraces;
  std::ofstream regTraces;

  VexRiscvTracer(VTote_sim_VexRiscv *cpu, bool trace_instruction, bool trace_reg) {
    this->cpu = cpu;
...
int main(int argc, char **argv) {
  ...
  TESTBENCH<VTote_sim> *tb = new TESTBENCH<VTote_sim>(TIMESCALE / SYSTEM_CLK_HZ);
  ...
  if (programPath) {
    ...
    size_t iram_max_size = sizeof(tb->dut->Tote_sim->tote->system_iRam->mem_symbol0) * 4;
    uint8_t *mem0 = (uint8_t *)tb->dut->Tote_sim->tote->system_iRam->mem_symbol0;
    uint8_t *mem1 = (uint8_t *)tb->dut->Tote_sim->tote->system_iRam->mem_symbol1;
    uint8_t *mem2 = (uint8_t *)tb->dut->Tote_sim->tote->system_iRam->mem_symbol2;
    uint8_t *mem3 = (uint8_t *)tb->dut->Tote_sim->tote->system_iRam->mem_symbol3;
    ...
    for (int i = 0; i < reader.segments.size(); ++i) {
      ...
      for (int i = 0; i < pseg->get_file_size(); i++) {
        ...
        switch ((i + start_addr) & 3) {
          case 0:
            mem0[(i + start_addr) / 4] = pseg->get_data()[i];
            break;
          case 1:
            mem1[(i + start_addr) / 4] = pseg->get_data()[i];
            break;
          case 2:
            mem2[(i + start_addr) / 4] = pseg->get_data()[i];
            break;
          case 3:
            mem3[(i + start_addr) / 4] = pseg->get_data()[i];
            break;
        }
      }
    }
  ...
  tb->add(new VexRiscvTracer(
      tb->dut->Tote_sim->tote->system_cpu,
  ...
  while (!tb->done()) {
    ...
    if (tb->dut->Tote_sim->tote->system_dBus_cmd_valid &&
        tb->dut->Tote_sim->tote->system_dBus_cmd_ready &&
        tb->dut->Tote_sim->tote->system_dBus_cmd_payload_write &&
        tb->dut->Tote_sim->tote->system_dBus_cmd_payload_address == 0xFFFFFFF8) {
      std::cout << static_cast<char>(tb->dut->Tote_sim->tote->system_dBus_cmd_payload_data);
    }
    if (tb->dut->Tote_sim->tote->system_cpu->lastStageIsFiring &&
        tb->dut->Tote_sim->tote->system_cpu->lastStagePc == exit_address) {
      exit_code = static_cast<int>(tb->dut->Tote_sim->tote->system_cpu->RegFilePlugin_regFile[10]);//a0
